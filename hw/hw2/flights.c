...
struct flightSys {
        struct airport_t* firstAirport;
};

struct airport {
	char *name;
	struct entry *firstEntry;
	struct airport_t *nextAirport;
};

struct entry {
	struct airport_t* dest;
	struct timeHM_t* departTime;
	struct timeHM_t* arriveTime;
	int cost;
...
flightSys_t* createSystem() {
    ...
    struct flightSys_t  *sys = malloc(sizeof(flightSys_t));
    if (!sys) {
        allocation_failed();
    }
    return sys;
...
void addAirport(flightSys_t* s, char* name) {
    // Replace this line with your code
   struct airport_t *newAir = malloc(sizeof(airport_t));
...
airport_t* getAirport(flightSys_t* s, char* name) {
    // Replace this line with your code
    struct airport_t *currentAir = *((*s).firstAirport);
    while (*currentAir) {
	if ( !strcomp( (currentAir).name , name) ){
 	    return currentAir;
	} else {
